...
export default function LoginAdminPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // interroge le rôle côté serveur via route util (sans fuite)
    const res = await fetch('/api/role', { cache: 'no-store' });
    const json = await res.json(); // { role: 'admin'|'instructor'|'tutor'|'learner'|null }
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-sm mx-auto mt-16 glass p-6 rounded-2xl space-y-4">
      <h1 className="text-2xl font-semibold text-iris-grad">Admin — Connexion</h1>
